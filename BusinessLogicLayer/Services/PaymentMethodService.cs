    ...
    public class PaymentMethodService : IPaymentMethodService
    {
        ...
        public async Task<int?> AddNewMethodAsync(CreatePaymentMethodRequest dto)
        {
            if (dto == null || !dto.IsValid())
                throw new BusinessException("Invalid payment method data.", 92000, Enums.ActionResult.InvalidData);

            var entity = PaymentMethodMap.ToEntity(dto);
            int? id = await _repo.AddNewMethod(entity);
            if (!id.HasValue)
                throw new BusinessException("Failed to create payment method.", 92002, Enums.ActionResult.DBError);
        ...
        {
            if (id <= 0)
                throw new BusinessException("Invalid payment method id.", 92000, Enums.ActionResult.InvalidData);

            var existing = await _repo.GetMethodByIdAsync(id);
            if (existing == null)
                throw new BusinessException("Payment method not found.", 92001, Enums.ActionResult.NotFound);
        ...
        public async Task<bool> UpdateMethodAsync(int id, UpdatePaymentMethodRequest dto)
        {
            ...
            if (!updated)
                throw new BusinessException("Failed to update payment method.", 92003, Enums.ActionResult.DBError);
        ...
        public async Task<bool> DeleteMethodAsync(int id)
        {
            ...
            if (!deleted)
                throw new BusinessException("Failed to delete payment method.", 92003, Enums.ActionResult.DBError);
